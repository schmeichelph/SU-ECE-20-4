...
def edge_sharpening(image):
    # blury = cv2.GaussianBlur(np.asarray(image),(5,5),0)
    # smoothing = cv2.addWeighted(blury,1.5,image,-0.5,0)
...
def filter_images(primary_image,image_source):
    ...
    while(i < 550):
        if(u[i][1][1] < 128 or u[i][1][1]>128):
            ...
            print("night")
            #hist_image = histogram_equalization(primary_image)
            ...
            cv2.imshow("final image", np.asarray(normal_img))
            cv2.waitKey(0)
            break
        i += 1
...
def match(primary_images, secondary_images, image_destination,
    ...
    #markov_cluster(np.asarray(descriptors))
    # num = 0
    # num = cv2.length(descriptors)
